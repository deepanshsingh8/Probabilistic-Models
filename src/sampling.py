...
outcomeSpace, prob_tables = prob_estimate.learn_bayes_net(prob_estimate.graphT, '../resources/bc_2.csv')
queryNodes = list(outcomeSpace.keys())
p = p_joint(outcomeSpace, prob_tables, queryNodes)
#########################
# Test code
#########################
prob_estimate.printFactor(query(p, outcomeSpace, ['BC'], {'Age': '50-74', 'Location': 'UpInQuad'}))

# Different Queries for the network

# Bayesian Network
prob_estimate.printFactor(query(p, outcomeSpace, ['LymphNodes', 'BC'], {'Metastasis': 'no'}))
...
for x in range(1000):
    s = sample(prob_estimate.graph, prob_tables)
